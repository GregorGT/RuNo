...
export default function Editor({
  ...
  const editorContainer = editorRef.current;
    ...
    const editor = useEditor({
      extensions, // Extensions are passed to the editor
      content: editorState[editorName] || defaultContent, // Set the content of the editor
      autofocus: 'end', // Focus at the end when the editor is initialized
      editable: true, // Set the editor to be editable
      editorProps: {
        handleScrollToSelection: (view) => {
          // Custom scroll behavior when the selection changes
          console.log('Editor is trying to scroll to the selection...');
          return true; // Return true to enable default behavior (scroll to selection)
        },
        attributes: {
          id: 'editor', // Set the editor's id
          style: `max-height:${height}px; overflow-y: scroll;`, // Apply styles directly here
        },
      },
    });
...
const handleScroll = (event: Event) => {
  ...
  return (
    <div>
      ...
      content={editorState[editorName] || defaultContent} // Pass the content to the editor
      autofocus="end" // Autofocus at the end of the content
    >
      {editor && <EditorContent editor={editor} />} {/* Render the EditorContent with the editor instance */}
    </EditorProvider>      </ErrorBoundary>
    </div>
  );
}
