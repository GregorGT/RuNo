...
useEffect(() => {
  ...
  //       className="tiptap"
  //       style={{ height: "400px", overflowY: "scroll" }} // Ensure it’s scrollable
  // >
  //       <EditorProvider
  //         editorProps={{
  //           attributes: {
  ...
  // );
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100vh" }}> {/* Make sure the parent has height 100vh */}
      {/* Only show the toolbar if `showToolbar` is true */}
      {showToolbar && <MenuBar editorName={editorName} />}

      {/* Scrollable area for editor content */}
      <div
        ref={editorRef} // Attach the ref to the wrapper div
        style={{
          maxHeight: `${height}px`, // Set max height for scrollable area
          overflowY: "auto", // Enable vertical scrolling
          flexGrow: 1, // Take up remaining space in the container
          border: "1px solid black", // Add a border for better visibility during debugging
        }}
      >
        <EditorProvider
          editorProps={{
