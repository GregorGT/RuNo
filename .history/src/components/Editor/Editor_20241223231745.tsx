...
return (
  <div
    style={{
      ...
      height: `${height}px`, // Parent container needs a height for layout
    }}
  >
    {/* Conditionally render the MenuBar using slotBefore */}
    <EditorProvider
      editorProps={{
        attributes: {
          id: "editor",
          style: `max-height:${height}px; overflow-y:auto;`, // Adjust the editor's height
        },
      }}
      editable={true}
      ...
      autofocus="end"
      slotBefore={showToolbar ? <MenuBar editorName={editorName} /> : null} // Place MenuBar in slotBefore
    >
      <div
        ref={editorRef} // Bind the ref to the editor container
        className="tiptap"
        style={{
          flexGrow: 1, // Make the editor take the remaining space
          overflowY: "auto", // Ensure the editor container is scrollable
          borderTop: "1px solid #ccc", // Optional: Add a border between MenuBar and editor
        }}
      >
        <ErrorBoundary>
          <></> {/* Editor content is managed by EditorProvider */}
        </ErrorBoundary>
      </div>
    </EditorProvider>
  </div>
