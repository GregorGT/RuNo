...
export default function Dropdowns() {
  ...
  const loadData = useAtomValue(loadEditorAtom);  // Get the loadEditorAtom's state
  ...
  const handleScroll = useCallback(() => {
    ...
    // Check and load next or previous chunk based on scroll position
    if (bottom) {
      if (currentChunkIndex < editorChunks.length - 1) {
        setCurrentChunkIndex((prev) => prev + 1);
        getEditorValue.load(editorChunks[currentChunkIndex + 1]);  // Load the next chunk
  
        // Sync other state when loading new chunk (this ensures that sorting, filters, etc. are reflected)
        // updateAdditionalStates();
      }
    } else {
      if (currentChunkIndex > 0) {
        setCurrentChunkIndex((prev) => prev - 1);
        getEditorValue.load(editorChunks[currentChunkIndex - 1]);  // Load the previous chunk
    ...
    // Ensure that the current chunk is updated with the latest content
    ...
    if (currentChunkContent !== editorChunks[currentChunkIndex]) {
      ...
      updatedChunks[currentChunkIndex] = currentChunkContent;  // Update the chunk content
