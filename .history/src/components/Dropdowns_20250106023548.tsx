...
const Dropdowns = React.forwardRef((props, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    ...
    handleEditorScroll: (editorRef: React.RefObject<HTMLDivElement>) => {
      ...
      // If the chunk has changed, we need to commit the current buffer
      if (chunkToLoad !== currentChunkIndex) {
        commitBufferToChunk(); // Commit the buffer before switching chunks
        ...
        getEditorValue.load(buffer.current); // Update editor with the buffer's content
      }
