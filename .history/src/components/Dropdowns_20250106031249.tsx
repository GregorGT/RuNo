  ...
  useImperativeHandle(ref, () => ({
    ...
    handleEditorScroll: (editorRef: React.RefObject<HTMLDivElement>) => {
      if (editorRef.current) {
        ...
        const currentChunkContent = getEditorValue.fn(); // Get the current chunk content from the editor
        if (currentChunkContent !== editorChunks[currentChunkIndex]) {
          const updatedChunks = [...editorChunks];
          updatedChunks[currentChunkIndex] = currentChunkContent; // Update the current chunk in state
          setEditorChunks(updatedChunks); // Update the state with the modified chunk
    
          console.log("Chunk updated:", currentChunkIndex, currentChunkContent); // Optional log to confirm the update
        }
