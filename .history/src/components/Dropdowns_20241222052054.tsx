...
export default function Dropdowns() {
  ...
  const [editorChunks, setEditorChunks] = useState<string[]>([]);
  ...
  const handleScroll = useCallback(() => {
    ...
    // Check if we reached the bottom of the editor
    ...
    // Ensure that when a chunk is scrolled out of view, it gets updated
    const currentChunkContent = getEditorValue.fn(); // Use the 'fn' method to get the content
    if (currentChunkContent !== editorChunks[currentChunkIndex]) {
      const updatedChunks = [...editorChunks];
      updatedChunks[currentChunkIndex] = currentChunkContent; // Update the chunk with new content
  ...
  const save_data = () => {
    // Create the full content by joining updated chunks
    const data = {
      editorData: editorChunks.join(''), // Join chunks into the full content
      filter: filterValue,
      sorting: sortingFn,
